...
public OnPluginStart()
{
	...
	lobbyAccess         = CreateConVar("l4d_vote_lobby_access",           "z",  "开始返回大厅投票所需的访问级别", CVAR_FLAGS);
	difficultyAccess    = CreateConVar("l4d_vote_difficulty_access",      "z",  "开始更改难度投票所需的访问级别", CVAR_FLAGS);
	levelAccess         = CreateConVar("l4d_vote_level_access",           "",  "开始更改地图投票所需的访问级别", CVAR_FLAGS);
	restartAccess       = CreateConVar("l4d_vote_restart_access",         "",  "启动重启地图投票所需的访问级别", CVAR_FLAGS);
	kickAccess          = CreateConVar("l4d_vote_kick_access",            "",  "开始投票踢人所需的访问级别", CVAR_FLAGS);
	kickImmunity        = CreateConVar("l4d_vote_kick_immunity",          "1", "Make votekick respect admin immunity", CVAR_FLAGS, true, 0.0, true, 1.0);
	vetoAccess          = CreateConVar("l4d_vote_veto_access",            "z", "否决投票所需的访问级别", CVAR_FLAGS);
	passVoteAccess      = CreateConVar("l4d_vote_pass_access",            "z", "通过投票所需的访问级别", CVAR_FLAGS);
	voteTimeout         = CreateConVar("l4d_vote_timeout",                "0", "玩家必须在投票之间等待(超时)这么多秒. 0 = no timeout", CVAR_FLAGS, true, 0.0);
	voteNoTimeoutAccess = CreateConVar("l4d_vote_no_timeout_access",      "",  "没有投票超时所需的访问级别.", CVAR_FLAGS);
	sendToLog           = CreateConVar("l4d_vote_log",                    "0", "Log voting data", CVAR_FLAGS, true, 0.0, true, 1.0);
	customAccess        = CreateConVar("l4d_custom_vote_access",          "z", "调用自定义投票所需的访问级别.", CVAR_FLAGS);
	voteNotify          = CreateConVar("l4d_vote_notify_access",          "",  "谁会看到某些与投票相关的通知. 如果空白每个人都看到他们.", CVAR_FLAGS);
	survivalMap         = CreateConVar("l4d_vote_surv_map_access",        "",  "切换生存地图所需的访问级别.", CVAR_FLAGS);
	survivalRestart     = CreateConVar("l4d_vote_surv_restart_access",    "",  "重新启动生存地图所需的访问级别.", CVAR_FLAGS);
	survivalLobby       = CreateConVar("l4d_vote_surv_lobby_access",      "z",  "在生存地图上返回大厅所需的访问级别.", CVAR_FLAGS);
	tankKickImmunity    = CreateConVar("l4d_vote_tank_kick_immunity",     "1", "使坦克免受投票踢.", CVAR_FLAGS, true, 0.0, true, 1.0);
