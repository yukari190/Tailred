...
void TriggerSpawn(L4D2_Infected desiredClass, float[3] pos)
{
	int kick = KickDeadInfectedBots();
	if (kick <= 0) // spawn immediately without delay
    {
   		CreateInfectedWithParams(view_as<L4D2_Infected>(desiredClass), pos);
	}
	else
...
void CreateInfectedWithParams(L4D2_Infected desiredClass, float[3] pos)
{
	int spawnedClient = -1;
	switch (desiredClass)
	{
