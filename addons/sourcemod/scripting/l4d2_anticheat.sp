...
public void OnPluginStart()
{
	...
	RegConsoleCmd("lgofnoc_clientsettings", _ClientSettings_Cmd, "列出由lgofnoc强制执行的客户端设置");
	RegServerCmd("lgofnoc_trackclientcvar", _TrackClientCvar_Cmd, "添加要由lgofnoc跟踪和实施的客户端CVar");
	RegServerCmd("lgofnoc_resetclientcvars", _ResetTracking_Cmd, "Remove all tracked client cvars. Cannot be called during matchmode");
	RegServerCmd("lgofnoc_startclientchecking", _StartClientChecking_Cmd, "开始检查并强制执行此插件跟踪的客户端cvar");
	
	SetConVarInt(CreateConVar("cl_consistencycheck_interval", "180.0", "自上次经过此时间(秒)后, 执行一致性检查.", FCVAR_REPLICATED|FCVAR_DEVELOPMENTONLY), 999999);
...
{
	if (!IsFakeClient(client)) ClientCommand(client, "cl_consistencycheck");
}

public void L4D2_OnPlayerTeamChanged(int client, int oldteam, int nowteam)
{
	if (!IsValidInGame(client)) return;
	if (nowteam == 3 && oldteam != 3) SDKHook(client, SDKHook_SetTransmit, Hook_SetTransmit);
	else if (nowteam != 3 && oldteam == 3) SDKUnhook(client, SDKHook_SetTransmit, Hook_SetTransmit);
...
{
	if (IsValidInGame(client) && IsValidInGame(entity) && GetClientTeam(client) == 3 && GetClientTeam(entity) == 2 && 
	GetEntProp(client, Prop_Send, "m_isGhost") == 1) return Plugin_Handled;
...
public int _EnforceCliSettings_QueryReply(QueryCookie cookie, int client, ConVarQueryResult result, const char[] cvarName, const char[] cvarValue, any value)
{
	...
	if (result)
	{
		LogMessage("[lgofnoc] ClientSettings: Couldn't retrieve cvar %s from %L, kicked from server", cvarName, client);
	...
	if ((clsetting.CLSE_hasMin && fCvarVal < clsetting.CLSE_min) || (clsetting.CLSE_hasMax && fCvarVal > clsetting.CLSE_max))
	{
		switch (clsetting.CLSE_action)
		{
			case CLSA_Kick:
			{
				LogMessage("[lgofnoc] ClientSettings: Kicking %L for bad %s value (%f). Min: %d %f Max: %d %f", \
			...
			case CLSA_Log:
			{
				LogMessage("[lgofnoc] ClientSettings: Client %L has a bad %s value (%f). Min: %d %f Max: %d %f", \
					client, cvarName, fCvarVal, clsetting.CLSE_hasMin, clsetting.CLSE_min, clsetting.CLSE_hasMax, clsetting.CLSE_max);
			}
		}
	}
