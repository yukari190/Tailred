	...
	if (fTimeLeft > 0) {//means fake god frames are in effect
		...
		if (IsTankRock(iInflictor)) {//tank rock
			if (g_hRock.BoolValue) {
				if (cTimerGod != CTimer_Null && IsValidTank(iAttacker))
				{
...
stock void ApplyDamageToSurvivor(int client, int Damage)
{
	...
	else if (Health == Damage)
	{
		if (TempHealth > 0)
		{
			...
			L4D_SetTempHealth(client, float(TempHealth - 1));
		}
		else if (TempHealth == 0)
		{
			SetEntityHealth(client, 1);
			L4D_SetTempHealth(client, 0.0);
			SetIncapacitated(client);
		}
	}
	else if (Health < Damage)
	{
		int temp = Damage - Health + 1;
		SetEntityHealth(client, 1);
		if (TempHealth > temp)
		{
			L4D_SetTempHealth(client, float(TempHealth - temp));
		}
		else
		{
			L4D_SetTempHealth(client, 0.0);
...
stock void SetIncapacitated(int client)
{
