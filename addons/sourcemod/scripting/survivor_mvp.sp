...
public int OnCvarChange(Handle convar, const char[] oldValue, const char[] newValue) 
{
...
{
	iGameMode = Gamemode();
    if (iGameMode == GAMEMODE_VERSUS) g_fMaxTankHealth = GetConVarFloat(g_hCvarTankHealth) * 1.5;
...
{
	if (!bReadyUpAvailable) bIsRoundLive = true;
	else bIsRoundLive = false;
...
public void L4D2_OnRealRoundEnd()
{
	...
	ClearTankDamage();
	CreateTimer(iGameMode == GAMEMODE_SCAVENGE ? 2.0 : 4.0, delayedMVPPrint);
}
...
public void L4D2_OnPlayerHurtPost(int victim, int attacker, int health, char[] Weapon, int damage, int dmgtype)
{
    ...
    if (IsValidInGame(victim) && IsValidInGame(attacker))
    {
        ...
        {
            zombieClass = GetInfectedClass(victim);
        ...
        else if (GetClientTeam(attacker) == TEAM_SURVIVOR && GetClientTeam(victim) == TEAM_SURVIVOR)
        {
            if (bIsRoundLive)
			{
	...
	if (!IsTank(victim) || IsPlayerIncap(victim)) return;
...
{
    if (IsValidInGame(attacker)) g_iDamage[attacker] += g_iLastTankHealth;
	if (g_bAnnounceTankDamage)
	{
		CPrintToChatAll("[{O}!{W}] 对 {B}Tank{W} ({G}%N{W}) 造成的{B}伤害", tankClient);
		PrintTankDamage();
	}
	ClearTankDamage();
}
...
public Action PlayerDeath_Event(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (IsValidInGame(victim) && IsValidInGame(attacker) && GetClientTeam(attacker) == TEAM_SURVIVOR)
    {
        zombieClass = GetInfectedClass(victim);
...
public Action InfectedDeath_Event(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (IsValidInGame(attacker) && GetClientTeam(attacker) == TEAM_SURVIVOR)
    {
        iGotCommon[attacker]++;
        iTotalCommon++;
    }
...
public Action SurvivorMVP_Cmd(int client, int args)
{
    ...
    if (IsValidInGame(client)) CPrintToChat(client, "{W}%s", printBuffer);
...
public Action ShowMVPStats_Cmd(int client, int args)
{
    if (client && IsClientConnected(client))
    {
        ...
        CPrintToChat(client, "{B}[{O}全场最佳{B}]{G} (你) 队友伤害: ({G}%d {W}队友伤害 [{O}%.0f%%{W}])\n", iDidFF[client], (float(iDidFF[client]) / float(iTotalFF)) * 100);
    }
}
...
public Action delayedMVPPrint(Handle timer)
{
    ...
    CPrintToChatAll("{W}%s", printBuffer);
    ...
    if (iTotalDamageAll > 0)
    {
        ...
        for (int i = 0; i <= 2; i++)
        {
            if (IsValidInGame(mvp_SI_losers[i])) {
    ...
    if (iTotalCommon > 0)
    {
        ...
        for (int i = 0; i <= 2; i++)
        {
            if (IsValidInGame(mvp_CI_losers[i])) {
    ...
    if (iTotalFF > 0)
    {
        ...
        for (int i = 0; i <= 2; i++)
        {
            if (IsValidInGame(mvp_FF_losers[i])) {
...
char GetMVPString()
{
    ...
    for (int j = 1; j <= teamCount; j++)
    {
        ...
        if (IsValidInGame(i) && IsClientConnected(i))
		{
...
int getSurvivor(int exclude[4])
{
    ...
    {
        if (IsConnectedAndInGame(i) && GetClientTeam(i) == TEAM_SURVIVOR)
        {
			int tagged = false;
			for (int j = 0; j < 4; j++)
			{
...
void PrintRemainingHealth()
{
	...
	CPrintToChatAll("[{O}!{W] {B}Tank{W} ({G}%s{W}) 还有 {G}%d{W} 的生命值 ", name, g_iLastTankHealth);
	PrintTankDamage();
}
...
void PrintTankDamage()
{
	...
	if ((percent_total < 100 && float(damage_total) > (g_fMaxTankHealth - (g_fMaxTankHealth / 200.0)))) percent_adjustment = 100 - percent_total;
...
{
	if (!IsInfectedAlive(client) || GetInfectedClass(client) != ZC_TANK) return false;
	return true;
}
