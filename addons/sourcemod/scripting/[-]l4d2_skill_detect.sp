...
stock HandleLevel( attacker, victim )
{
    ...
    if ( GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{O}★★★ {G}%N {B}fully {W}leveled {G}%N", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{O}★★★ {G}%N {B}fully {W}leveled {G}a charger", attacker );
        }
    }
...
stock HandleLevelHurt( attacker, victim, damage )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{O}★ {G}%N {B}chip-leveled {G}%N {W}({B}%i dmg{W})", attacker, victim, damage );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{O}★ {G}%N {B}chip-leveled {W}a charger ({B}%i dmg{W})", attacker, damage );
        }
    }
...
stock HandleDeadstop( attacker, victim )
{
    ...
    /*if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{O}★ {G}%N {B}deadstopped {G}%N", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{O}★ {G}%N {B}deadstopped {G}a hunter", attacker );
        }
    }*/
...
HandleTongueCut( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{O}★★★ {G}%N {B}cut {G}%N{W}'s tongue", attacker, victim );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{O}★★ {G}%N {B}cut {W}smoker tongue", attacker );
        }
    }
...
HandleSmokerSelfClear( attacker, victim, bool:withShove = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)) 
    {
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
        {
            CPrintToChatAll( "{O}★★ {G}%N {B}self-cleared {W}from {G}%N{W}'s tongue{B}%s", attacker, victim, (withShove) ? " by shoving" : "" );
        }
        else if ( IS_VALID_INGAME(attacker) )
        {
            CPrintToChatAll( "{O}★★ {G}%N {B}self-cleared {W}from a smoker tongue{B}%s", attacker, (withShove) ? " by shoving" : "" );
        }
    }
...
HandleRockSkeeted( attacker, victim )
{
    ...
    if (GetConVarBool(g_hCvarReport))
    {
        ...
        */
        CPrintToChatAll( "{O}★ {G}%N {B}skeeted {W}a tank rock", attacker );
    }
...
stock HandleHunterDP( attacker, victim, actualDamage, Float:calculatedDamage, Float:height, bool:playerIncapped = false )
{
    ...
    if (GetConVarBool(g_hCvarReport)
        ...
        {
            CPrintToChatAll( "{O}★★ {G}%N {R}high-pounced {G}%N {W}({R}%i {W}dmg, height: {R}%i{W})", attacker,  victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            CPrintToChatAll( "{O}★★ {G}A hunter {R}high-pounced {G}%N {W}({R}%i {W}dmg, height: {R}%i{W})", victim, RoundFloat(calculatedDamage), RoundFloat(height) );
        }
    }
...
stock HandleJockeyDP( attacker, victim, Float:height )
{
    ...
    if (    GetConVarBool(g_hCvarReport)
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            CPrintToChatAll( "{O}★★★ {G}%N {R}high-pounced {G}%N {W}({R}height{W}: {R}%i{W})", attacker,  victim, RoundFloat(height) );
        }
        else if ( IS_VALID_INGAME(victim) )
        {
            CPrintToChatAll( "{O}★★★ {G}A jockey {R}high-pounced {G}%N {W}({R}height{W}: {R}%i{W})", victim, RoundFloat(height) );
        }
    }
...
stock HandleDeathCharge( attacker, victim, Float:height, Float:distance, bool:bCarried = true )
{
    ...
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(attacker) )
        {
            CPrintToChatAll( "{O}★★★★ {G}%N {R}death-charged {G}%N{W} %s({R}height{W}: {R}%i{W})",
                    attacker,
                    victim,
                    (bCarried) ? "" : "by bowling ",
                    RoundFloat(height)
        ...
        {
            CPrintToChatAll( "{O}★★★★ {G}A charger {R}death-charged {G}%N{W} %s({R}height{W}: {R}%i{W})",
                    victim,
                    (bCarried) ? "" : "by bowling ",
                    RoundFloat(height) 
                );
    ...
    if ( attacker != pinVictim && GetConVarBool(g_hCvarReport))
    {
        ...
        if ( fClearTime != -1.0 && fClearTime <= fMinTime )
        {
            if ( IS_VALID_INGAME(attacker) && IS_VALID_INGAME(victim) && !IsFakeClient(victim) )
            {
                ...
                {
                    CPrintToChatAll( "{O}★ {G}%N {B}insta-cleared {G}%N {W}from {G}%N{W}'s %s ({B}%.2f {W}seconds)",
                            attacker, pinVictim, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                    CPrintToChatAll( "{O}★ {G}%N {B}insta-cleared {G}a teammate {W}from {G}%N{W}'s %s ({B}%.2f {W}seconds)",
                            attacker, victim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
            ...
            else if ( IS_VALID_INGAME(attacker) )
            {
                if ( IS_VALID_INGAME(pinVictim) )
                {
                    CPrintToChatAll( "{O}★ {G}%N {B}insta-cleared {G}%N {W}from a %s ({B}%.2f {W}seconds)",
                            attacker, pinVictim,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
                } else {
                    CPrintToChatAll( "{O}★ {G}%N {B}insta-cleared {G}a teammate {W}from a %s ({B}%.2f {W}seconds)",
                            attacker,
                            g_csSIClassName[zombieClass],
                            fClearTime
                        );
...
stock HandleBHopStreak( survivor, streak, Float: maxVelocity )
{
    if (    GetConVarBool(g_hCvarReport) &&
        ...
        CPrintToChat(survivor, "{O}★ {G}You {W}got {B}%i bunnyhop%s {W}in a row ({B}top speed: {G}%.1f{W})",
                streak,
                ( streak > 1 ) ? "s" : "",
                maxVelocity
            );
