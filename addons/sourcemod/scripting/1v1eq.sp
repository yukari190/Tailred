...
public Action Event_PlayerHurt(Event event, const char[] name, bool dontBroadcast)
{
	...
	int zombie_class = GetInfectedClass(attacker);

	if (GetClientTeam(attacker) == 3 && zombie_class != 8 && damage >= 24)
	{
		int remaining_health = GetClientHealth(attacker);
		CPrintToChatAll("[{olive}EQ 1v1{default}] {red}%N{default}({green}%s{default}) had {olive}%d{default} health remaining!", attacker, L4D2_InfectedNames[zombie_class], remaining_health);
...
public Action PlayerDeath_Event(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (attacker != iClientPlaying)                             { return; }     // ignore shots fired by anyone but survivor player
    if (!IsValidInGame(victim))                             { return; }     // safeguard
    if (GetClientTeam(victim) != TEAM_INFECTED)                 { return; }     // safeguard
...
public Action InfectedDeath_Event(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (attackerId && IsValidInGame(attacker))
    {
...
public Action AbilityUse_Event(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if(IsValidInGame(user) && strcmp(abilityName,"ability_lunge",false) == 0 && !bIsPouncing[user])
    {
...
{
    if(IsValidInGame(client) && (isGrounded(client) || !IsPlayerAlive(client)))
    {
...
char PrintSkeetStats(int toClient)
{
	...
	} else if (IsValidInGame(toClient)) {
