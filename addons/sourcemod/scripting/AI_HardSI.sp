...
{
	name = "AI: Hard SI",
	author = "Breezy, High Cookie, Standalone, Newteee, Yukari190",
...
ConVar
	...
	hTankDamage,
	hLungeInterval,
	hJockeyLeapAgainTimer,
	hTongueDelay,
	hSmokerHealth,
	hChokeDamageInterrupt,
	hTankAttackRange,
	hTongueRange,
	hVomitRange;
...
bool
	...
	bHasQueuedLunge[MAXPLAYERS+1],
	bCanLunge[MAXPLAYERS+1],
	bHasHunterShoved[MAXPLAYERS+1],
	bHasJockeyShoved[MAXPLAYERS+1], // shoved jockeys will stop hopping
	bCanLeap[MAXPLAYERS+1],
	bDoNormalJump[MAXPLAYERS+1]; // used to alternate pounces and normal jumps

int
	bShouldCharge[MAXPLAYERS+1],
	g_targetSurvivor[MAXPLAYERS+1], // survivor target of each special infected
	g_state[MAXPLAYERS+1][8];

float
	throwForce[MAXPLAYERS + 1][3],
	fLungeInterval,
	fJockeyLeapAgainTimer,
	g_L4D2Infected_attack_time,
	fTankAttackRange,
	fToungeRange,
	fVomitRange,
	g_move_grad[MAXPLAYERS+1][3],
	g_move_speed[MAXPLAYERS+1],
	g_pos[MAXPLAYERS+1][3],
	g_delay[MAXPLAYERS+1][8],
	fInterval,
	fOverkill[2048];
...
public void OnPluginStart()
{
	...
	hGasCanSpitTime = FindConVar("gascan_spit_time");
	hTankDamage = FindConVar("vs_tank_damage");
	hLungeInterval = FindConVar("z_lunge_interval");
	hJockeyLeapAgainTimer = FindConVar("z_jockey_leap_again_timer");
	hTankAttackRange = FindConVar("tank_attack_range");
	hTongueRange = FindConVar("tongue_range");
	hVomitRange = FindConVar("z_vomit_range");
	
	hGasCanSpitTime.AddChangeHook(ConVarChange);
	hLungeInterval.AddChangeHook(ConVarChange);
	hJockeyLeapAgainTimer.AddChangeHook(ConVarChange);
	hTankAttackRange.AddChangeHook(ConVarChange);
	hTongueRange.AddChangeHook(ConVarChange);
	hVomitRange.AddChangeHook(ConVarChange);
	
	ConVarChange(null, "", "");
	
	hSmokerHealth = FindConVar("z_gas_health");
	hChokeDamageInterrupt = FindConVar("tongue_break_from_damage_amount"); 
	hTongueDelay = FindConVar("smoker_tongue_delay"); 
	
	hSmokerHealth.AddChangeHook(OnTongueCvarChange);
	hChokeDamageInterrupt.AddChangeHook(OnTongueCvarChange);
	hTongueDelay.AddChangeHook(OnTongueCvarChange);
...
{
	hChokeDamageInterrupt.RestoreDefault();
	hTongueDelay.RestoreDefault();
}

public void ConVarChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
	fInterval = hGasCanSpitTime.FloatValue;
	fLungeInterval = hLungeInterval.FloatValue;
	fJockeyLeapAgainTimer = hJockeyLeapAgainTimer.FloatValue;
	fTankAttackRange = hTankAttackRange.FloatValue;
	fToungeRange = hTongueRange.FloatValue;
	fVomitRange = hVomitRange.FloatValue;
}

public void OnTongueCvarChange(ConVar convar, const char[] oldValue, const char[] newValue)
{
	hTongueDelay.SetFloat(SMOKER_TONGUE_DELAY);	
	hChokeDamageInterrupt.SetInt(hSmokerHealth.IntValue);
}
...
{
	if (!IsValidTank(client) || !IsPlayerAlive(client)) return Plugin_Stop;
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if (GetVectorDistance(survivorPos, tankPos) < 120)
		{
			...
			throwForce[i][0] = L4D2Util_ClampFloat((360000.0 * (survivorPos[0] - tankPos[0])), -400.0, 400.0);
			throwForce[i][1] = L4D2Util_ClampFloat((90000.0 * (survivorPos[1] - tankPos[1])), -400.0, 400.0);
			throwForce[i][2] = 300.0;
			SurvivorNearTank[i] = true;
		}
		else
		{
			SurvivorNearTank[i] = false;
...
public Action PlayerTeam_Event(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		SDKUnhook(client, SDKHook_OnTakeDamage, OnTakeDamage2);
	}
}

public Action OnTakeDamage2(int victim, int &attacker, int &inflictor, float &damage, int &damageType)
{
	if (!IsValidSurvivor(victim) || !IsPlayerAlive(victim) || !IsValidTank(attacker) || !IsPlayerAlive(attacker)) return Plugin_Continue;
	char classname[64];
	...
	if (StrContains(classname, "tank_claw", false) != -1)
	{
		...
		{
			if (!IsClientInGame(i) || GetClientTeam(i) != 2 || !IsPlayerAlive(i)) continue;
			if (i == victim || !SurvivorNearTank[i]) continue;
			
			if (!IsIncapacitated(i)) SetAnimFling(i, attacker, throwForce[i]);
			SDKHooks_TakeDamage(i, attacker, attacker, hTankDamage.FloatValue, DMG_GENERIC);
		}
	}
	return Plugin_Continue;
}
...
{
	int client = GetClientOfUserId(event.GetInt("userid"));
	if (!IsBotInfected(client)) return Plugin_Continue;
...
public Action OnPlayerShoved(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (!IsBotInfected(shovedPlayer)) return Plugin_Continue;
	...
	if (GetEntProp(client, Prop_Send, "m_hasVisibleThreats") && 1500 > iSurvivorsProximity > 250)
	{
		if (currentspeed > 200.0)
		{
			if (flags & 1)
			{
				...
				{
					Client_Push(client, clientEyeAngles, BoostForward, {VelocityOvr_None,VelocityOvr_None,VelocityOvr_None});
				}
				if (buttons & IN_BACK)
				{
...
void Client_Push(int client, float clientEyeAngle[3], float power, int override[3] = VelocityOvr_None)
{
...
int GetTargetSurvivor()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if (GetSurvivorIncapCount(i) < 1 && IsMobile(i))
		{
